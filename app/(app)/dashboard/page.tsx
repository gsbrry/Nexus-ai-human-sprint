...
export default function DashboardPage() {
  ...
  const today = new Date('2025-06-11'); // pin demo date so visuals stay consistent
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-8 py-8 space-y-8">
      {!isAuthConfigured() && (
        <div className="rounded-lg border border-gold/30 bg-gold/5 px-5 py-3">
      ...
      <div className="flex items-start justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight">Hey {me.name.split(' ')[0]}.</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat label="Blockers" value={blockers.length.toString()} sub={blockers.length ? 'needs attention' : 'none'} tone={blockers.length ? 'red' : undefined} />
        <Stat label="Points done" value={`${donePoints}/${totalPoints}`} sub={`${Math.round((donePoints / Math.max(1, totalPoints)) * 100)}% complete`} tone="gold" />
      </div>
  ...
  return (
    <Card className="bg-[#0A0A0A]">
      <CardContent className="p-5">
        ...
        <div
          className={
            'font-mono text-[26px] font-extrabold mt-1 ' +
